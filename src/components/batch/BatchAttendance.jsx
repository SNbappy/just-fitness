...
const STATUSES = [
  { value: "present", label: "P", icon: Check, cls: "bg-primary-500 text-white", idle: "hover:bg-primary-50 text-primary-600" },
    ...
    return (
      <div>
        <div className="card p-5 flex flex-col sm:flex-row gap-4 sm:items-end justify-between">
          <div>
            <label className="text-xs font-bold uppercase tracking-wider text-ink-400">Session Date</label>
        ...
        {Object.keys(marks).length > 0 && (
          <div className="mt-4 flex gap-3 text-xs font-bold">
            <span className="px-3 py-1.5 rounded-lg bg-primary-50 text-primary-700">Present {counts.present || 0}</span>
        ...
        {roster.length === 0 ? (
          <div className="card p-10 text-center mt-5 text-ink-500">No members to mark yet.</div>
        ) : (
          <div className="mt-5 card divide-y divide-ink-100">
            {roster.map((m) => (
              <div key={m.id} className="p-4 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-semibold text-ink-900 text-sm truncate">{m.profiles.full_name}</p>
                  <p className="text-xs text-ink-400 truncate">{m.profiles.department || "—"}</p>
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={`grid place-items-center w-10 h-10 rounded-xl font-bold text-sm transition-all ${
                          active ? s.cls + " scale-105" : "bg-ink-50 " + s.idle
                        }`}
                        title={s.value}
                      >
                        <s.icon size={17} />
  ...
  return (
    <div>
      <div className="grid grid-cols-3 gap-4">
        <StatCard icon={TrendingUp} label="Attendance" value={history.length ? `${rate}%` : "—"} color="bg-primary-50 text-primary-600" />
        <StatCard icon={Flame} label="Current Streak" value={streak} color="bg-secondary-50 text-secondary-600" />
      ...
      {history.length === 0 ? (
        <div className="card p-10 text-center mt-5">
          <CalendarDays className="mx-auto text-ink-300" size={34} />
          <p className="mt-3 font-semibold text-ink-600">No attendance recorded yet</p>
          <p className="text-sm text-ink-400 mt-1">Your trainer marks attendance after each session.</p>
        </div>
      ) : (
        <div className="mt-5 card p-5">
          <p className="text-xs font-bold uppercase tracking-wider text-ink-400 mb-4">Recent Sessions</p>
